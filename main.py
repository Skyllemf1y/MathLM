...
from config import CHAPITRES, NOM_APPLICATION, NB_QUESTIONS_PAR_NIVEAU, NB_QUESTIONS_TEST, VIES_INITIALES, PSEUDOS_ADMIN
from modules.game_logic import PartieEnCours
from modules.progression import (
    niveau_est_debloque, niveau_est_valide, dernier_niveau_valide,
    marquer_cours_vu, cours_est_vu, meilleure_note_test,
)
...
from modules.comptes import creer_compte, verifier_identifiants, compter_comptes
from modules.badges import calculer_badges
from modules.invite import generer_pseudo_invite, est_invite
from modules.erreurs import obtenir_points_faibles
from modules.stats_admin import obtenir_statistiques_globales, compter_joueurs
...
def admin_requis(fonction):
    @wraps(fonction)
    def fonction_protegee(*args, **kwargs):
        if session.get("pseudo_compte") not in PSEUDOS_ADMIN:
...
@app.context_processor
def injecter_est_invite():
    return {"utilisateur_est_invite": est_invite(session.get("pseudo_compte"))}


@app.context_processor
def injecter_est_admin():
    return {"utilisateur_est_admin": session.get("pseudo_compte") in PSEUDOS_ADMIN}
...
def classement():
    ...
    return render_template(
        "classement.html", titre_page="Classement",
        entrees=entrees, pseudo_actuel=session.get("pseudo_compte"),
    )
...
def jouer_en_invite():
    ...
    if pseudo_actuel and not est_invite(pseudo_actuel):
        flash(f"Tu es déjà connecté en tant que {pseudo_actuel}.", "erreur")
        return redirect(url_for("accueil"))

    pseudo_invite = generer_pseudo_invite()
    session["pseudo_compte"] = pseudo_invite
    flash("Tu joues en tant qu'invité. Pense à créer un compte pour ne pas perdre ta progression !", "succes")
    return redirect(url_for("accueil"))
...
def chapitre(chapitre_id):
    ...
    etats_niveaux = [
        {"numero": n, "valide": niveau_est_valide(pseudo, chapitre_id, n), "debloque": niveau_est_debloque(pseudo, chapitre_id, n)}
        for n in niveaux
    ]
    return render_template("chapitre.html", chapitre_id=chapitre_id, nom_chapitre=infos["nom"], titre_page=infos["nom"], etats_niveaux=etats_niveaux)
...
def cours(chapitre_id):
    ...
    return render_template("cours.html", chapitre_id=chapitre_id, nom_chapitre=infos["nom"], titre_page=infos["nom"] + " · Cours", cours=contenu_cours)
...
def voir_progression():
    ...
    for chapitre_id, infos in CHAPITRES.items():
        ...
        resume_chapitres.append({
            "chapitre_id": chapitre_id, "nom": infos["nom"], "nb_valides": nb_valides, "nb_total": nb_total,
            "pourcentage": round(100 * nb_valides / nb_total),
            "cours_vu": cours_est_vu(pseudo, chapitre_id), "note_test": meilleure_note_test(pseudo, chapitre_id),
        })

    points_faibles_bruts = obtenir_points_faibles(pseudo, list(CHAPITRES.keys()), top=3)
    points_faibles = [
        {"chapitre_id": pf["chapitre_id"], "nom": CHAPITRES[pf["chapitre_id"]]["nom"], "pourcentage_erreur": round(pf["taux_erreur"] * 100)}
        for pf in points_faibles_bruts
    ]

    return render_template("progression.html", titre_page="Ma progression", resume_chapitres=resume_chapitres, points_faibles=points_faibles)
...
def profil():
    ...
    return render_template(
        "profil.html", titre_page="Profil", avatars_disponibles=AVATARS_DISPONIBLES,
...
def jouer(chapitre_id, niveau):
    ...
    return render_template(
        "game.html", chapitre_id=chapitre_id, nom_chapitre=CHAPITRES[chapitre_id]["nom"], titre_page=CHAPITRES[chapitre_id]["nom"],
        niveau=niveau, est_test=False, est_challenge=False, url_retour=url_for("chapitre", chapitre_id=chapitre_id),
        question=partie.question_actuelle["texte"], vies=partie.vies, vies_max=VIES_INITIALES, score=partie.score,
        objectif=NB_QUESTIONS_PAR_NIVEAU, nb_bonnes_reponses=0,
    )
...
def test_chapitre(chapitre_id):
    ...
    session["partie"] = partie.vers_dict()
    return render_template(
        "game.html", chapitre_id=chapitre_id, nom_chapitre=CHAPITRES[chapitre_id]["nom"], titre_page=CHAPITRES[chapitre_id]["nom"] + " · Test",
        niveau=None, est_test=True, est_challenge=False, url_retour=url_for("chapitre", chapitre_id=chapitre_id),
        question=partie.question_actuelle["texte"], vies=partie.vies, vies_max=VIES_INITIALES, score=partie.score,
        objectif=NB_QUESTIONS_TEST, nb_bonnes_reponses=0,
    )
...
def challenge():
    ...
    session["partie"] = partie.vers_dict()
    return render_template(
        "game.html", chapitre_id=None, nom_chapitre="Mode Challenge", titre_page="Challenge infini",
        niveau=None, est_test=False, est_challenge=True, url_retour=url_for("accueil"),
        question=partie.question_actuelle["texte"], vies=partie.vies, vies_max=VIES_INITIALES, score=partie.score,
        objectif=None, nb_bonnes_reponses=0, meilleur_score_challenge=obtenir_meilleur_score_challenge(pseudo),
    )
...
def admin_stats():
    return render_template(
        "admin_stats.html", titre_page="Statistiques (admin)",
        stats=obtenir_statistiques_globales(), nb_joueurs=compter_joueurs(), nb_comptes=compter_comptes(),
    )
